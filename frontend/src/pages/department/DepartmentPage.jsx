...
export default function DepartmentPage() {
  ...
  const openAdd = () => { setEditTarget(null); setForm({ name: '', location_id: '' }); setError(''); setShowForm(true) }
  const openEdit = dept => { setEditTarget(dept); setForm({ name: dept.name, location_id: dept.location_id || '' }); setError(''); setShowForm(true) }
  const closeForm = () => { setShowForm(false); setEditTarget(null); setForm({ name: '', location_id: '' }); setError('') }
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/departments/${deleteTarget.id}`); setDeleteTarget(null); fetchData() }
    ...
    <div className="w-full space-y-6">
      ...
      <Card className="p-0 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12 text-sm text-gray-400">Belum ada departemen.</div>
        ) : (
          <div>
            {departments.map((dept, idx) => (
              ...
              >
                ...
                <div className="flex gap-1 flex-shrink-0">
                  <button onClick={() => openEdit(dept)} className="text-xs px-2.5 py-1.5 rounded-lg text-gray-500 hover:bg-gray-100 transition-colors">Edit</button>
          ...
          <div>
            ...
            {locations.length === 0 ? (
              ...
              <>
                ...
                <p className="text-xs text-gray-400 mt-1.5">Jika dipilih, karyawan hanya bisa absen dalam radius lokasi ini.</p>
              </>
            )}
          </div>
      ...
      <ConfirmModal
        ...
        message={deleteTarget ? `Departemen "${deleteTarget.name}" akan dihapus dari sistem.` : ''}
