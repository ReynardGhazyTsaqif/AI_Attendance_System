...
export default function SchedulePage() {
  ...
  const scheduledDeptIds = dataReady
    ? schedules.map((s) => Number(s.department_id))
    : [];
  const availableDepts = dataReady
    ? departments.filter((d) => !scheduledDeptIds.includes(Number(d.id)))
    : [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      editTarget
        ? await api.put(`/schedules/${editTarget.id}`, {
            name: form.name,
            work_start: form.work_start,
            work_end: form.work_end,
            late_tolerance: Number(form.late_tolerance),
            early_leave_tolerance: Number(form.early_leave_tolerance),
          })
      ...
      setError(err.response?.data?.detail || "Gagal menyimpan jadwal");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">
            Atur jam masuk dan pulang per departemen
          </p>
        </div>
          ...
          {schedules.map((s) => (
            <Card key={s.id}>
              <div className="flex items-start justify-between mb-3">
                ...
                <span
                  className={`text-[11px] px-2 py-0.5 rounded-full flex-shrink-0 ${s.is_active ? "bg-green-50 text-green-700" : "bg-gray-50 text-gray-400"}`}
                >
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => openEdit(s)}
                  className="flex-1 text-xs py-2 rounded-lg bg-gray-50 hover:bg-gray-100 text-gray-600 transition-colors"
                >
                  Edit
                </button>
                <button
                  onClick={() => setDeleteTarget(s)}
                  className="flex-1 text-xs py-2 rounded-lg bg-red-50 hover:bg-red-100 text-red-500 transition-colors"
                >
                  Hapus
                </button>
              </div>
      ...
      <FormModal
        ...
        title={
          editTarget
            ? `Edit Jadwal — ${editTarget.department_name}`
            : "Tambah Jadwal"
        }
        onClose={closeForm}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {!editTarget && (
            <div>
              ...
              {availableDepts.length === 0 ? (
                <p className="text-sm text-amber-700 bg-amber-50 px-3 py-3 rounded-lg">
                  Semua departemen sudah memiliki jadwal.
                </p>
              ) : (
                <select
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, work_start: e.target.value })
                }
                required
                className={inputClass}
              />
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, late_tolerance: e.target.value })
                }
                className={inputClass}
              />
              <p className="text-xs text-gray-400 mt-1.5">
                Hadir s.d. {addTime(form.work_start, form.late_tolerance)}
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1.5">
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, early_leave_tolerance: e.target.value })
                }
                className={inputClass}
              />
              <p className="text-xs text-gray-400 mt-1.5">
                Normal setelah{" "}
                {subTime(form.work_end, form.early_leave_tolerance)}
              </p>
            </div>
          </div>
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button
              type="submit"
              disabled={
                submitting || (!editTarget && availableDepts.length === 0)
              }
      ...
      <ConfirmModal
        ...
        message={
          deleteTarget
            ? `Jadwal "${deleteTarget.name}" untuk ${deleteTarget.department_name} akan dihapus.`
            : ""
        }
