...
export default function SchedulePage() {
  ...
  const handleReset = () => { setShowForm(false); setEditTarget(null); setError(''); setForm({ department_id: '', name: '', work_start: '08:00', work_end: '17:00', late_tolerance: 15, early_leave_tolerance: 15 }) }
      ...
      {showForm && (
        <Card>
          <p className="text-sm font-medium text-gray-800 mb-4">
            {editTarget ? `Edit Jadwal — ${editTarget.department_name}` : 'Jadwal Baru'}
          </p>
          <form onSubmit={handleSubmit} className="space-y-4">
            {error && <div className="text-sm text-red-600 bg-red-50 px-3 py-2.5 rounded-lg">{error}</div>}
            {!editTarget && (
              <div>
                <label className="block text-xs text-gray-500 mb-1.5">Departemen</label>
                {availableDepts.length === 0 ? (
                  <p className="text-sm text-amber-700 bg-amber-50 px-3 py-2.5 rounded-lg">Semua departemen sudah memiliki jadwal.</p>
                ) : (
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1.5">Nama Jadwal</label>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-xs text-gray-500 mb-1.5">Jam Pulang</label>
                <input type="time" value={form.work_end} onChange={e => setForm({ ...form, work_end: e.target.value })} required className={inputClass} />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs text-gray-500 mb-1.5">Toleransi Terlambat <span className="text-gray-300">(menit)</span></label>
                ...
                <p className="text-[11px] text-gray-400 mt-1">Dianggap hadir s.d. {addTime(form.work_start, form.late_tolerance)}</p>
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1.5">Toleransi Pulang Cepat <span className="text-gray-300">(menit)</span></label>
                ...
                <p className="text-[11px] text-gray-400 mt-1">Normal jika pulang setelah {subTime(form.work_end, form.early_leave_tolerance)}</p>
              </div>
            </div>
            <div className="flex justify-end gap-2">
              <Button variant="secondary" onClick={handleReset} size="sm">Batal</Button>
              <Button type="submit" disabled={submitting || (!editTarget && availableDepts.length === 0)} size="sm">
          ...
          {schedules.map(s => (
            <Card key={s.id}>
              <div className="flex items-start justify-between mb-3">
                ...
                <span className={`text-[11px] px-2 py-0.5 rounded-full ${s.is_active ? 'bg-green-50 text-green-700' : 'bg-gray-50 text-gray-400'}`}>
          ...
          <div className="space-y-2">
            {availableDepts.map(d => (
              <div key={d.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                ...
                <button
                  onClick={() => { setForm(prev => ({ ...prev, department_id: String(d.id) })); setShowForm(true); setEditTarget(null) }}
