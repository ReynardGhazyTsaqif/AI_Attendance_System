...
export default function AttendancePage() {
  ...
  useEffect(() => {
    api
      .get(`/attendance/today/${user.user_id}`)
      .then((r) => setTodayAttendance(r.data))
      .catch(() => {});
    api
      .get("/schedules/my")
      .then((r) => setMySchedule(r.data))
      .catch(() => {});
  }, [user]);
  ...
  const capture = useCallback(async () => {
    ...
    try {
      ...
      const endpoint =
        mode === "checkin" ? "/attendance/check-in" : "/attendance/check-out";
    ...
    <div className="space-y-6 w-full">
      <div>
        ...
        <p className="text-sm text-gray-400 mt-0.5">
          Kedipkan mata untuk verifikasi, lalu scan otomatis
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-4">
          <p className="text-xs text-gray-400 mb-2 uppercase">Status</p>

          <span
            className={`inline-flex px-3 py-1 rounded-full text-xs font-medium ${
              statusStyle[todayAttendance?.status] || "bg-gray-50 text-gray-500"
            }`}
          >
        ...
        <Card className="p-4">
          <p className="text-xs text-gray-400 mb-2 uppercase">Check In</p>
        ...
        <Card className="p-4">
          <p className="text-xs text-gray-400 mb-2 uppercase">Check Out</p>
        ...
        <Card className="p-4">
          <p className="text-xs text-gray-400 mb-2 uppercase">Jadwal</p>

          <p className="text-2xl font-semibold text-gray-900">
            {mySchedule
              ? `${mySchedule.work_start?.slice(
                  0,
                  5,
                )} - ${mySchedule.work_end?.slice(0, 5)}`
              : "--"}
          </p>

          {mySchedule && (
            <p className="text-xs text-gray-400 mt-1">
              Tol. +{mySchedule.late_tolerance} / -
              {mySchedule.early_leave_tolerance}
            </p>
          )}
        </Card>
      </div>
      ...
      {!location && !locationError && (
        <div className="text-sm text-amber-700 bg-amber-50 border border-amber-100 px-4 py-3 rounded-xl">
          Menunggu lokasi GPS. Tombol absensi akan aktif setelah lokasi tersedia.
        </div>
      )}
      {location && !mode && (
      ...
      {!mode && (
        <div className="grid grid-cols-2 gap-3">
          <button
            ...
            disabled={!location || !!todayAttendance?.check_in_time}
          ...
          {!livenessCleared && (
            <div>
              <div className="bg-violet-50 border border-violet-100 text-violet-700 text-sm px-4 py-3 rounded-lg mb-4">
                Kedipkan mata 2× untuk membuktikan kamu bukan bot — kamera akan
                aktif otomatis.
              </div>
              <LivenessCheck
                onPassed={() => {
          ...
          <div className="text-center space-y-1.5">
            ...
            {result.confidence_score && (
              <p className="text-sm text-green-600">
                Akurasi: {result.confidence_score.toFixed(1)}%
              </p>
            )}
