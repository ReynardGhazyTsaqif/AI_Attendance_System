...
export default function LocationPage() {
  ...
  const handleReset = () => { setShowForm(false); setEditTarget(null); setForm({ name: '', latitude: '', longitude: '', radius_meter: 100 }); setError('') }
  const handleEdit = loc => { setEditTarget(loc); setForm({ name: loc.name, latitude: loc.latitude, longitude: loc.longitude, radius_meter: loc.radius_meter }); setShowForm(true); setError('') }
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {showForm && (
        <Card>
          <p className="text-sm font-medium text-gray-800 mb-4">{editTarget ? `Edit — ${editTarget.name}` : 'Lokasi Baru'}</p>
          <form onSubmit={handleSubmit} className="space-y-4">
            {error && <div className="text-sm text-red-600 bg-red-50 px-3 py-2.5 rounded-lg">{error}</div>}
            <div>
              <label className="block text-xs text-gray-500 mb-1.5">Nama Lokasi</label>
              <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} required placeholder="contoh: Kampus Utama, Kantor Pusat" className={inputClass} />
            </div>
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs text-gray-500">Koordinat GPS</label>
                <button type="button" onClick={handleGetCurrentLocation} disabled={gettingLocation} className="text-xs text-violet-600 hover:text-violet-700 disabled:opacity-50 transition-colors">
              ...
              {form.latitude && form.longitude && (
                <button type="button" onClick={() => window.open(`https://www.google.com/maps?q=${form.latitude},${form.longitude}`, '_blank')} className="text-xs text-violet-500 hover:text-violet-700 mt-1.5 transition-colors">
                  Cek di Google Maps →
                </button>
              )}
            </div>
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs text-gray-500">Radius Absensi</label>
                <span className="text-xs font-medium text-violet-600">{form.radius_meter} m</span>
              </div>
              <input type="range" min="50" max="500" step="10" value={form.radius_meter} onChange={e => setForm({ ...form, radius_meter: e.target.value })} className="w-full accent-violet-600" />
          ...
          locations.map(loc => (
            <Card key={loc.id}>
              ...
              <div className="font-mono text-xs text-gray-400 bg-gray-50 rounded-lg px-3 py-2 mb-3 flex items-center justify-between">
                ...
                <button onClick={() => window.open(`https://www.google.com/maps?q=${loc.latitude},${loc.longitude}`, '_blank')} className="text-violet-500 hover:text-violet-700 non-mono text-xs transition-colors">
                  Maps
                </button>
              </div>
              <div className="mb-4">
