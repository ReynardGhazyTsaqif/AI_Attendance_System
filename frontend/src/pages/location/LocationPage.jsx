...
export default function LocationPage() {
  ...
  const openAdd = () => {
    setEditTarget(null);
    setForm({ name: "", latitude: "", longitude: "", radius_meter: 100 });
    setError("");
    setShowForm(true);
  };
  const openEdit = (loc) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        radius_meter: parseInt(form.radius_meter),
      };
      editTarget
      ...
      setError(err.response?.data?.detail || "Gagal menyimpan lokasi");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="w-full space-y-6">
              ...
              <div className="mb-4">
                <div className="w-full bg-gray-100 rounded-full h-1">
                  <div
                    ...
                    style={{
                      width: `${Math.min((loc.radius_meter / 500) * 100, 100)}%`,
                    }}
                  />
                </div>
              </div>
      ...
      <ConfirmModal
        ...
        message={
          deleteTarget
            ? `Lokasi "${deleteTarget.name}" akan dihapus dari sistem.`
            : ""
        }
